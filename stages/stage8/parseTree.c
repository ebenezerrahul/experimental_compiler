...
void generateVirtualFunTable(ClassTable *currClass, int vbase) {
  Memberfunclist *currFunction = currClass->Mfuncptr;
  int vptr = vbase;
  fprintf(target_file, "ADD SP, 8\n");
  int p = get_reg();
  fprintf(target_file, "MOV R%d, SP\n", p);
  fprintf(target_file, "SUB R%d, 8\n", p);
  while (currFunction != NULL) {
    fprintf(target_file, "MOV [R%d], LABEL%d\n", p, currFunction->Flabel);
    fprintf(target_file, "ADD R%d, 1\n", p);
    currFunction = currFunction->next;
  }
  free_reg();
  // currClass->VfuncAddr = vptr;
};

void generateAllVirFunTables() {
  ClassTable *curr = getAllClassTable();
  int offset = 0;
  while (curr) {
    generateVirtualFunTable(curr, bindAddr(0) + 8 * offset);
    offset++;
    curr = curr->next;
  }
}
...
void genCodeStmt(Snode *curr) {
  if (!curr) {
    ...
    free_reg();
    free_reg();
  } else if (curr->stmt_type == breakpoint) {
    ...
    free_reg();
    free_reg();
  } else if (curr->stmt_type == new_statement) {
    int temp = get_reg();
    ...
    fprintf(target_file, "ADD R15, 7\n");
    fprintf(target_file, "MOV [R15], %d\n",
            ClassTableLookUp(curr->currType->name)->VfuncAddr);
    free_reg();
...
int evaluateExpr(ExprNode *curr) {
  if (!curr) {
    ...
    if (curr->hasval) {
      ...
      // call the function
      int call_reg = get_reg();
      fprintf(target_file, "ADD R%d,7\n", selfReg);
      fprintf(target_file, "MOV R%d,[R%d]\n", call_reg, selfReg);
      fprintf(target_file, "ADD R%d, %d\n", call_reg,
              currFunction->FunctionOffset);
      fprintf(target_file, "MOV R%d,[R%d]\n", selfReg, call_reg);
...
int checkArgumentsMeathod(Memberfunclist *currFunc, struct ArgList *currArgs) {
  ParamList *curr = currFunc->paramlist->next;
  currArgs = currArgs->next;
  while (currArgs != NULL && curr != NULL) {
    if (curr->data_type != currArgs->expr->data_type)
      return 0;
    curr = curr->next;
    currArgs = currArgs->next;
  }
  return curr == NULL && currArgs == NULL;
}
