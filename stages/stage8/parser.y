...
%token BEG END INT STR STRING DECLBEG ENDDECL MAIN RETURN TYPEDEFBEG TYPEDEFEND null_const INITIALIZE ALLOC FREE CLASS ENDCLASS NEW EXTENDS BREAKPONIT
...
PRIVATEDECLBLOCK                :  FEILD_DEFINATION_LIST 
                                {
                                    if(Cptr->Parentptr != NULL) {
                                        ...
                                        FeildsTable *last = getLastFeild($<feilds>1);
                                            printf("HI wow\n");
                                        if(last != NULL) 
                                        {
...
CLASSNAME                       : ID 
                                ...
                                | ID EXTENDS ID 
                                {
                                    Cptr = ClassTableInstall($<str>1, $<str>3);
                                }
                                ;
...
MEATHODDEF                      : TYPE ID '(' PARAMLIST ')' '{' LDECLBLOCK BODY '}'
                                {
                                    ...
                                    if(currFunction == NULL || currFunction->data_type != $<type>1 || matchParameters(currFunction->paramlist->next,currParams->next) == 0) {
...
GDECL                           : TYPE GidLIST ';'
                                {
                                    ...
                                    ClassTable *currClass = searchClassTable($<type>1->name);
                                    if(currClass != NULL) 
                                        setClassType($<decl>2, currClass);
                                }
                                ;
...
ALLOCSTATEMENT                  :  REFERENCE '=' ALLOC '(' ')' ';' 
                                ...
                                | REFERENCE '=' NEW '(' TYPE ')' ';'
                                {
                                    ...
                                    temp.reference = $<expr>1;
                                    ClassCompatibleCheck($<expr>1->symbol->classType, ClassTableLookUp($<type>5->name));
                                    $<snode>$ = createSnode(temp);
                                    $<snode>$->currType = $<type>5;
                                }
                                ;
...
ASSIGN_STATEMENT                : REFERENCE '=' EXPR 
                                {
                                    ...
                                    temp.expr = $<expr>3;
                                    temp.reference = $<expr>1;
                                    ClassTable *A = searchClassTable($<expr>1->data_type->name);
                                    ClassTable *B = searchClassTable($<expr>3->data_type->name);
                                    if((A != NULL && B != NULL))
                                    {
                                        ClassCompatibleCheck(A,B);
                                    }
                                    else if($<expr>1->data_type != $<expr>3->data_type && $<expr>3->data_type != TypeLookUp("null"))
                                    {
