...
void countDuplicates(FeildsTable *curr, char *name) {
  int count = 0;
  while (curr != NULL) {
    if (strcmp(curr->name, name) == 0)
      count++;
...
FeildsTable *getLastFeild(FeildsTable *head) {
  FeildsTable *curr = head;
  FeildsTable *prev = NULL;
  while (curr != NULL) {
    prev = curr;
    curr = curr->next;
  }
...
void checkForDuplicateFeilds(FeildsTable *head) {
  FeildsTable *curr = head;
  while (curr != NULL) {
    countDuplicates(head, curr->name);
    curr = curr->next;
  }
};
...
FeildsTable *feildsLookup(TypeTable *currType, char *FeildName) {
  ...
  printf("No Such Feild for Var %s", FeildName);
  exit(1);
  return NULL;
}

FeildsTable *copyFeildsTable(FeildsTable *curr) {
  FeildsTable *cpy, *prev, *retVal;
  cpy = NULL;
  retVal = NULL;
  prev = NULL;
  int count = 0;
  while (curr != NULL) {
    cpy = malloc(sizeof(FeildsTable));
    *cpy = *curr;
    curr = curr->next;
    cpy->next = NULL;
    if (prev != NULL)
